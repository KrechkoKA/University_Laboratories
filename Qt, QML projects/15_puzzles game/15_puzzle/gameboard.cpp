...
namespace
{
bool isAbjacent (const GameBoard::Position f, const GameBoard::Position s){
    if (f == s)
        return false;
      ...
      m_boardSize {m_dimension * m_dimension}
{
    ...
    std::iota(m_rawBoard.begin(), m_rawBoard.end(), 1);         //генерация последовательности
    shuffle();
}
...
bool GameBoard::move(int index)
{
    ...
    auto hiddenElementIterator = std::find(m_rawBoard.begin(), m_rawBoard.end(), boardSize());  //hidden element
    ...
    if (!isAbjacent(elementPosition, hiddenElementPosition))
        return false;
...
{
    Q_UNUSED(parent)
    return m_rawBoard.size();                   //возвращаем кол-во элементов на доске
}
...
void GameBoard::shuffle()
{
    ...
    do {
      std::shuffle(m_rawBoard.begin(), m_rawBoard.end(), generator);    //перемешивание последовательности
    } while(!isBoardValid());
}
